...
LFP::LFP(int numWorkers) : shutdownFlag(false), currentLeader(0) {
    for (int i = 0; i < numWorkers; ++i) {  // Create threads and add them to the workerThreads vector
...
void LFP::taskProcessor(int workerId) {
    while (true) {  // Keep processing tasks until the shutdown flag is set
        ...
        
        {
            ...
            // Check if the worker should stop processing
            {
                ...
                if (shutdownFlag && taskQueue.empty()) return;  // Exit if shutdown is true and no tasks remain
            }
