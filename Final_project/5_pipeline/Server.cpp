...
void initializePAO()
{
    std::vector<std::function<void(void *)>> functions = {
        ...
        [](void *taskPtr)
        {
            ...
            // if (send(task->clientFd, task->msg.c_str(), task->msg.size(), 0) < 0)
            // {
            //     perror("send");
            // }
            // delete task->mstGraph;
            // delete task;
        }};
...
void Server::start()
{
    ...
    // signal(SIGINT, handleSig); // Register signal handler for graceful shutdown
...
void Server::solveMSTWithPipeline(int client_socket, MSTAlgorithmType algoType, const std::string algorithm)
{
    ...
    if (solver)
    {
        ...
        Triple *task = new Triple{&mstResults[client_socket], "MST created using " + algorithm + " algorithm.\n", client_socket};

        clientTasks[client_socket] = task;

        pao->enqueueTask(static_cast<void *>(task));
