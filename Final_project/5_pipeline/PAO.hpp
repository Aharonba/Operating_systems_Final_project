...
private:
    ...
    struct Worker
    {
        std::thread *workerThread;                // The worker thread
        ...
        std::mutex *queueLock;                    // Mutex for the task queue
        std::condition_variable *queueCondition;  // Condition variable for worker synchronization
